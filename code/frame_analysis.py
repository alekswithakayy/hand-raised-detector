...
for index,face_location in enumerate(face_locations):
    ...
    person_left = max((face_left - face_width * 2), 0)

    # You can access the actual face itself like this:
    person_array = image[person_top:person_bottom, person_left:person_right]
    person_pil_img = Image.fromarray(person_array)

    person_pil_img.save('./video_frames/' + str(index) + '.jpg')
    person_pil_img.show()
    clarifai_image = ClImage(file_obj=open('./video_frames/' + str(index) + '.jpg', 'rb'))
